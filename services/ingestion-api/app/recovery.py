...
RECONCILE_INTERVAL_SECONDS = float(
    os.environ.get("QUEUE_RECONCILE_INTERVAL_SECONDS", "15")
)
...
def mark_published(document_id, published_at: datetime | None = None) -> bool:
...
def _unpublished_batch() -> list[tuple[object, datetime]]:
    with Session(get_engine()) as session:
        docs = session.exec(
            select(Document)
...
async def reconcile_once(js) -> tuple[int, int]:
    ...
    if pending:
        ...
        metrics.queue_oldest_unpublished_seconds.set(
            max(0.0, (_utcnow() - oldest).total_seconds())
        )
    else:
    ...
    for document_id, _created_at in pending:
        try:
            ...
            metrics.queue_publish_total.labels(
                source="reconciler", outcome="acknowledged"
            ).inc()
            published += 1
        except Exception:
            metrics.queue_publish_total.labels(
                source="reconciler", outcome="failed"
            ).inc()
            failed += 1
            logger.exception(
                "failed to reconcile queued document %s; will retry", document_id
            )
    return published, failed


async def reconcile_forever(js) -> None:
    while True:
        try:
