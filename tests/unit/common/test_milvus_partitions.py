...
class TestScopedLifecycle:
    def test_delete_is_partition_scoped(self, fake: FakeMilvusClient) -> None:
        store = MilvusStore()
        store.upsert([_point("d1", 0, "CUI"), _point("d1", 0, "SECRET")])
        store.delete_document_chunks("d1", "CUI")
        assert "d1-0" not in fake.partitions["cls_cui"]
        assert "d1-0" in fake.partitions["cls_secret"]
