...
class TestReleasabilityClause:
    def test_none_restriction_is_always_included(self):
        ...
        assert (
            f'array_contains_any(releasability, ["{NO_RELEASABILITY_RESTRICTION}"])' in expr
        )
...
class TestInjectionResistance:
    def test_quote_in_group_name_cannot_break_out(self):
        hostile = 'x" or status != "'
        expr = build_access_expr(
            _claims(groups=[hostile]), allowed_classifications=ALLOWED
        )
    ...
    def test_backslash_is_escaped_before_quote(self):
        expr = build_access_expr(
            _claims(groups=['trailing\\"']), allowed_classifications=ALLOWED
        )
        assert '\\\\\\"' in expr
